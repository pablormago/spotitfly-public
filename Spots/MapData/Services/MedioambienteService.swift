...
struct MedioambienteService {
    // ===== BEGIN REPLACE: MedioambienteService.fetchAround (WFS → REST) =====
    static func fetchAround(location: CLLocation, km: Double = 10) async throws -> [ENAIREFeature] {
        ...
        #if DEBUG
        print("🌍 MedioambienteService(REST) → URL:\n\(url.absoluteString)")
        #endif
        ...
        #if DEBUG
        print("✅ MedioambienteService(REST) → \(decoded.features.count) crudas, dentro spot: \(dentro.count)")
        #endif
